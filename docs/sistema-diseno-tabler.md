...
- En las tarjetas de modelos de Inicio, la potencia máxima y el conector se combinan por tipo de carga para evitar filas redundantes: `mdi-ev-plug-type2`, `AC · Tipo 2`, límite en kW y `Corriente alterna`; `mdi-ev-plug-ccs2`, `DC · CCS2`, límite en kW y `Corriente continua`. Los iconos usan `2rem`, cada conjunto se centra y ambos permanecen uno al lado del otro cuando hay espacio. No repetir aparte `Carga máxima AC`, `Carga rápida DC` ni `Conectores AC y DC`.

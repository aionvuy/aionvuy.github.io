...
- En las tarjetas de modelos de Inicio, la potencia máxima y el conector se combinan por tipo de carga para evitar filas redundantes. Cada conjunto presenta, en este orden: icono de `2rem`, `Carga AC` o `Carga DC`, significado de la sigla (`Corriente alterna` o `Corriente continua`), valor en kW destacado, etiqueta `Potencia máxima` y `Conector Tipo 2` o `Conector CCS2`. AC y DC permanecen en dos columnas centradas y se separan mediante un divisor vertical tenue. No mostrar siglas, conectores ni cifras sin su significado o unidad, ni repetir aparte `Carga máxima AC`, `Carga rápida DC` o `Conectores AC y DC`.
- Cuando una bandera con nombre accesible ya identifica inequívocamente una opción de navegación, no agregar además un MDI temático: evitar dos símbolos equivalentes antes de la misma etiqueta.
